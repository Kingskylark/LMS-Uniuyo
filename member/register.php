	...
	<body>
		<form class="cd-form" method="POST" action="#">
			<center><legend>Member Registration</legend><p>Please fillup the form below:</p></center>
				...
				<div class="icon">
					<input class="m-reg_num" type="text" name="m_reg_num" id="m_reg_num" placeholder="Registration Number" required />
				</div>

				<div class="icon">
					<input class="m-department" type="text" name="m_department" id="m_department" placeholder="Department" required />
				</div>

				<div class="icon">
					<input class="m-department" type="number" name="m_balance" id="m_balance" placeholder="Balance" required />
				</div>
		...
		{		
			...
			$query = $con->prepare("(SELECT username FROM member WHERE username = ?) UNION (SELECT username FROM pending_registrations WHERE username = ?);");
				...
				else
				{
					...
					{
						$query = $con->prepare("INSERT INTO pending_registrations(username, password, name, email, reg_num, department, balance) VALUES(?, ?, ?, ?, ?, ?, ?);");
						$query->bind_param("ssssssd", $_POST['m_user'], ($_POST['m_pass']), $_POST['m_name'], $_POST['m_email'], $_POST['m_reg_num'], $_POST['m_department' ], $_POST['m_balance']);
						if($query->execute())
